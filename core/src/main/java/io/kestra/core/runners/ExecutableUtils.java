...
import io.kestra.core.trace.ExecutionTextMapSetter;
...
public final class ExecutableUtils {
    ...
    public static <T extends Task & ExecutableTask<?>> Optional<SubflowExecution<?>> subflowExecution(
        ...
        var tracer = ((DefaultRunContext) runContext).getApplicationContext().getBean(Tracer.class);
        var propagator = openTelemetry.getPropagators().getTextMapPropagator();

        var span = tracer.spanBuilder("EXECUTOR - " + currentTask.getType())
            .setAllAttributes(TraceUtils.attributesFrom(currentExecution))
            .startSpan();
